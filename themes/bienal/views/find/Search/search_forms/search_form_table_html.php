...
?>
					<!--
					#2 Estamos retirando esse campo da busca avançada.
					FRED 5/2/2021
					<div class="campo">        
                        ...
                        <input type="hidden" name="idno_label" value="fundo" />
                    </div>
                    -->
					
                    <div class="campo" style="margin-top:5px; margin-left:2px">
                        ...
                        <div>
							...
							<!--
							#4 Estamos retirando esse campo da busca avançada.
							Só precisamos de 4 níveis de busca
							FRED 5/2/2021
...
<script>
	...
	$("#ca_occurrence_labels_name").autocomplete({
		source: function( $request, $response ) {
			$.ajax( {
				url: "/service.php/Find/ca_occurrences?q=ca_occurrences.name:" + $request.term + "&limit=10",
				success: function( $data ) {
	...
	$("#ca_entity_labels_displayname").autocomplete({
		source: function( $request, $response ) {
			$.ajax( {
				url: "/service.php/Find/ca_entities?q=ca_entities.displayName:" + $request.term + "&limit=10",
				success: function( $data ) {
	...
	$("#ca_place_labels_name").autocomplete({
		source: function( $request, $response ) {
			$.ajax( {
				url: "/service.php/Find/ca_places?q=ca_places.name:" + $request.term + "&limit=10",
				success: function( $data ) {
