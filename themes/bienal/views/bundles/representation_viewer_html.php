 ...
 */
	...
	if ($vs_mimetype ==  'application/pdf') {
		switch($vs_pdf_pref = $this->request->user->getPreference('pdf_viewer')) {
			case 'native_plus_book_viewer':
				if (Session::getVar('has_pdf_plugin')) { 
	...
	if ($vb_use_book_reader) {
		...
		$va_page_cache = Session::getVar('caDocumentViewerPageListCache');
		...
		Session::setVar('caDocumentViewerPageListCache', $va_page_cache);
		
		$va_section_cache = Session::getVar('caDocumentViewerSectionCache');
		$va_section_cache[$vn_object_id.'/'.$vn_representation_id] = $va_sections;
		Session::setVar('caDocumentViewerSectionCache', $va_section_cache);
